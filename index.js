...
class CacheManager {
  ...
  getTarballPath(packageName, version) {
    return path.join(this.cacheDir, `${packageName}-${version}.tgz`);
  }

  getMetadataPath(packageName) {
    return path.join(this.cacheDir, `${packageName}-metadata.json`);
  }
...
class PackageInstaller {
  ...
  async installPackage(pkg) {
    ...
    try {
      ...
      if (this.cache.hasTarball(pkg.name, pkg.version)) {
        ...
        // Download tarball
        const tempTarball = path.join(TEMP_DIR, `${pkg.name}-${pkg.version}.tgz`);
  ...
  async runLifecycleScripts(packages) {
    ...
    for (const pkg of packages) {
      if (pkg.scripts?.install) {
        try {
          const pkgPath = path.join(NODE_MODULES, pkg.name);
          await execAsync(pkg.scripts.install, { cwd: pkgPath });
  ...
  printStats() {
    ...
    log(`  Failed: ${this.stats.failed} packages`, this.stats.failed > 0 ? 'warn' : 'info');
...
class SnappCLI {
  ...
  async addCommand(args) {
    if (args.length === 0) {
      log('Usage: snapp add <package[@version]> [--dev]', 'error');
      process.exit(1);
    }
  ...
  async listCommand(args) {
    ...
    log(`📦 Installed packages (${packages.length}):`, 'info');
    for (const pkg of packages) {
  ...
  helpCommand() {
    console.log(`
╔═══════════════════════════════════════════════════╗
║   SNAPP v2.0 - Simple Node App Package Manager   ║
...
Commands:
  ...
  install [options]   Install dependencies from package.json
    --production      Install only production dependencies
    --force           Clear cache and reinstall
  add <pkg> [opts]    Add a package to dependencies
...
Environment Variables:
  SNAPP_REGISTRY      Custom registry URL
  SNAPP_PARALLEL      Max parallel downloads (default: 10)
  SNAPP_DEBUG         Enable debug logging
